...
class SpeedometerApp:
    ...
    def draw_speedometer(self):
        # Draw circle
        self.canvas.create_arc(50, 50, 250, 250, start=180, extent=180, outline="black", width=2)

        # Draw markings
        for angle in range(180, 0, -10):
            x1 = 150 + 100 * cos(angle * pi / 180)
            y1 = 150 + 100 * sin(angle * pi / 180)
            x2 = 150 + 90 * cos(angle * pi / 180)
            y2 = 150 + 90 * sin(angle * pi / 180)
            self.canvas.create_line(x1, y1, x2, y2, fill="black")

            # Add numbers
            if angle % 20 == 0:
                x_text = 150 + 110 * cos(angle * pi / 180)
                y_text = 150 + 110 * sin(angle * pi / 180)
                self.canvas.create_text(x_text, y_text, text=str(180 - angle), fill="black")

    def update_dial(self, cpm):
        angle = 180 - min(cpm / 10, 100) * 1.8  # 1.8 degrees per CPM
